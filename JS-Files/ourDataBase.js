...
class DataHandler {
  ...
  static init() {
    try {
      ...
      let reservation = JSON.parse(Reservation);
      for (let key in reservation) {
        DataHandler.insertReservation(
          key,
          reservation[key].email,
          reservation[key].id,
          reservation[key].startDate,
          reservation[key].endDate,
          reservation[key].startTime,
          reservation[key].endTime
        );
      }

      let bussiness = JSON.parse(BussinessAccount);
      for (let key in bussiness)
        DataHandler.insertBussinessAccount(
          bussiness[key].email,
          bussiness[key].PageName,
          bussiness[key].PhoneNumber,
          bussiness[key].BussinessType,
          key
        );
    } catch (err) {
      console.error("Error reading JSON files:", err);
  ...
  static insertUser(_email, username, password, type) {
    if (
      DataHandler.userMap.get(_email.trim().toLowerCase()) != undefined &&
      !DataHandler.isUpdate
    )
      return;
  ...
  static insertEvent(
    ...
    if (id == undefined) id = DataHandler.eventMap.size;
  ...
  static insertBussinessAccount(email, pageName, phoneNumber, type, key) {
    if (key == undefined) key = "buss-" + DataHandler.eventMap.size;
  ...
  static updateEvent(
    ...
    if (id != undefined) {
      ...
      let newDate = date != undefined && date != "" ? date.trim() : row.date;
      let newVenue =
        venue != undefined && venue != "" ? venue.trim() : row.venueId;
      let newtime = time != undefined && time != "" ? time.trim() : row.time;
      let newtheme =
        theme != undefined && theme != "" ? theme.trim() : row.theme;
      let newDescription =
        Description != undefined && Description != ""
          ? Description.trim()
          : row.Description;
      let newCount =
        Count != undefined && Count != "" ? Count.trim() : row.Count;
      let newtype = type != undefined && type != "" ? type.trim() : row.type;
      DataHandler.insertEvent(
        id,
        newName,
